...
public class HomeController {
    ...
    public String afficherAccueil(
        ...
        @RequestParam(value = "categorie", required = false) List<String> categorie,
    ...
    System.out.println("Type Annonce: " + typeAnnonce);
        ...
        List<Annonce> annonces = annonceRepository.findAll().stream()
                ...
                .filter(a -> (categorie == null || 
                        categorie.stream()
                                 .anyMatch(cat -> a.getCategorie() != null && 
                                                  a.getCategorie().stream()
                                                            .anyMatch(c -> c != null && 
                                                                          c.toLowerCase().contains(cat.toLowerCase())))))

                // Filtrer par type de bien si il n'est pas null et si l'annonce a un type qui correspond
                .filter(a -> (typeAnnonce == null || 
                        (a.getBien() != null && a.getBien().getType() != null && 
