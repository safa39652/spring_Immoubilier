...
public String afficherAccueil(
    ...
    List<Annonce> toutesLesAnnonces = annonceRepository.findAll();
    System.out.println("Toutes les annonces récupérées: " + toutesLesAnnonces.size());

    // Appliquer les filtres
    List<Annonce> annonces = toutesLesAnnonces.stream()
            ...
            .filter(a -> (categorie == null || a.getCategorie().contains(categorie)))
            .filter(a -> (typeAnnonce == null || a.getTitre().toLowerCase().contains(typeAnnonce.toLowerCase())))
            .collect(Collectors.toList());

    System.out.println("Annonces après filtrage: " + annonces.size());
    annonces.forEach(annonce -> System.out.println(annonce));
