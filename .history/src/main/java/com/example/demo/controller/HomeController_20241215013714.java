...
public String afficherAccueil(
    ...
    List<Annonce> annonces = annonceRepository.findAll().stream()
            ...
            // Filter based on localisation in the 'categorie' list
            .filter(a -> (localisation == null || a.getCategorie().stream()
                    .anyMatch(c -> c.toLowerCase().contains(localisation.toLowerCase()))))
            // Filter based on localisation in the 'bien' object (assuming 'bien' has a 'getType' method)
            .filter(a -> (localisation == null || (a.getBien() != null && a.getBien().getType() != null 
                    && a.getBien().getType().toLowerCase().contains(localisation.toLowerCase()))))
